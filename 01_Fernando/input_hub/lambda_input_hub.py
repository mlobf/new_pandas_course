...
object_links = prefix_event.replace('input_crm','links').replace(f'_{date_proccess}','')
########################################################################################

########################################################################################
#leitura do arquivo input_hub e links
input_crm = pd.read_csv(f"{bucket_name}{prefix_event}",encoding = "ISO-8859-1",sep = ";")
links = pd.read_csv(f"{bucket_name}{object_links}",encoding = "ISO-8859-1",sep = ";")
########################################################################################
...
data_prep =input_crm[['Cluster','Data'\
	,'Dia da Semana'\
	,'UTM_CAMPAIGN'\
	,'UTM_SOURCE'\
	,'UTM_MEDIUM'\
	...
	.rename(columns = {'Data':'DATA_ENVIO'\
...
#TIPO_BANNER
transpose =pd.melt(data_prep,id_vars=['Cluster'\
	,'TIPO_ENVIO'\
	,'DATA_ENVIO'\
	,'DIA_SEMANA'\
	,'UTM_CAMPAIGN'\
...
transpose=transpose.rename(columns = {'Value' : 'NOME_BANNER'}).sort_values(by=['Cluster','TIPO_BANNER'], ascending=False)
########################################################################################

########################################################################################
#Ajuste nomenclatura Banner
transpose["TIPO_BANNER"] = transpose['TIPO_BANNER'].replace({'Principal' : 'PRINCIPAL'})
...
transpose["TIPO_BANNER"] = transpose['TIPO_BANNER'].replace({'Reengajamento' : 'ENGAJAMENTO'})
transpose["TIPO_BANNER"] = transpose['TIPO_BANNER'].replace({'Extra' : 'EXTRA'})
########################################################################################

########################################################################################
#join com tabela de links
join_links = pd.merge(transpose, links, how="left", left_on='NOME_BANNER', right_on='Nome')
########################################################################################

########################################################################################
#ajuste do layout final do input_hub
input_hub=join_links[['Cluster'\
,'TIPO_ENVIO'\
,'DATA_ENVIO'\
,'DIA_SEMANA'\
,'UTM_CAMPAIGN'\
...
,'Texto Legal'\
,'COMENTARIOS'\
,'LOGO_LINK']].rename(columns = {'Link' : 'URL','Filtros':'FILTRO','Texto Legal':'TEXTO_LEGAL'})
########################################################################################
...
for i in cluster:
	...
	write_df.\
		to_csv(f"{bucket_name}{prefix_event.replace(file,'').replace('input_crm','input_hub')}{file.replace('input_crm','input_hub')}",sep = ';', index=False ,encoding='utf-16')
